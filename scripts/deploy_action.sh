...
git config --local user.email "RoCodedev@nts-corp.com"
git config --local user.name "RoCode Dev"
...
cd build
git config user.name "RoCode Dev"
git config user.email "RoCodedev@nts-corp.com"
...
then
    ...
    git add .
    git commit -m "RoCode Js PUBLISH by v3.$DATEFMT.${GITHUB_RUN_NUMBER}"
...
#    git push --force --quiet "https://${GH_TOKEN}@${GH_REF}" build --tags
    ...
    git add .
    git commit -m "RoCode Js deploy $deployName"
    #git push --force --quiet "https://${GH_TOKEN}@${GH_REF}" "$deployName"
fi
#curl -d '{"tag_name": "v$DATEFMT","target_commitish": "build","name": "v$DATEFMT","body": "Description of the release","draft": false,"prerelease": false}' -X POST "https://developer.github.com/v3/repos/kimokim/RoCodejs/releases"
